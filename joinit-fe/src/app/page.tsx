...
export default async function Home() {
  return (
    <div className="min-h-screen bg-white dark:from-gray-800 dark:to-gray-900 text-gray-800 dark:text-gray-200 pt-16">
      ...
      {/* Hero Section */}
      <section className="bg-white dark:bg-gray-800 text-center py-20 rounded-md mx-6 mt-6 flex flex-col items-center">
      ...
      <section className="container mx-auto py-12 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4">
        {['landpage1.jpg', 'landpage1.jpg','landpage1.jpg', 'landpage1.jpg','landpage1.jpg', 'landpage1.jpg',].map((image, index) => (
          <div key={index} className="relative overflow-hidden rounded-md shadow-lg hover:scale-105 transform transition-transform">
            <Image src={`/images/${image}`} width={300} height={300} alt={`Evento ${index + 1}`} className="object-cover w-full h-full rounded-md" />
